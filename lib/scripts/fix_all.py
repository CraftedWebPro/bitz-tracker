#!/usr/bin/env python3
"""Fix all three issues in action_whatsapp_dialog."""
...
# New function with:
# 1. Canvas + Scrollbar for scrollable preview
# 2. Fixed send_all loop (iterates businesses correctly)
# 3. Confirmation + action_mark_sent() called properly
new_func = '''    def action_whatsapp_dialog(self):
        ...
        prev_canvas = tk.Canvas(preview_outer, bg=config.COLORS["surface"], highlightthickness=0)
        prev_scrollbar = tk.Scrollbar(preview_outer, orient="vertical", command=prev_canvas.yview)
        prev_scrollable = tk.Frame(prev_canvas, bg=config.COLORS["surface"])

        prev_scrollable.bind("<Configure>", lambda e: prev_canvas.configure(scrollregion=prev_canvas.bbox("all")))
        prev_canvas.create_window((0,0), window=prev_scrollable, anchor="nw")
        prev_canvas.configure(yscrollcommand=prev_scrollbar.set)

        prev_canvas.pack(side="left", fill="both", expand=True, padx=(15,0), pady=15)
        prev_scrollbar.pack(side="right", fill="y", pady=15, padx=(0,15))

        prev_text = tk.Text(prev_scrollable, font=("Segoe UI", 11), 
...
print("3. Mark Sent called properly after sending")
