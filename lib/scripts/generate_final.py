#!/usr/bin/env python3
"""Generate and apply the correct action_whatsapp_dialog function."""
...
# 2. Fixed send_all loop using enumerate() properly
# 3. Proper action_mark_sent() calls
# 4. Lambda functions fixed
...
new_func = '''    def action_whatsapp_dialog(self):
        ...
        if len(businesses) > 1:
            ...
            def go_prev():
                current_idx.set(max(0, current_idx.get()-1))
                update_review()
            def go_next():
                current_idx.set(min(len(businesses)-1, current_idx.get()+1))
                update_review()
            
            tk.Button(nav_frame, text="< Prev", bg=config.COLORS["surface2"], fg=config.COLORS["text"],
                     font=("Segoe UI", 9), padx=10, pady=4,
                     command=go_prev).pack(side="left", padx=(0,5))
            tk.Button(nav_frame, text="Next >", bg=config.COLORS["surface2"], fg=config.COLORS["text"],
                     font=("Segoe UI", 9), padx=10, pady=4,
                     command=go_next).pack(side="left", padx=5)
